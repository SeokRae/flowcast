...
> **In English** — flowcast is a Claude Code plugin for working with interface/service flow diagrams end to end. Give it data (text, a `.pptx` deck, or a description); a router splits it into diagram units and picks a view, then drawer subagents fan out in parallel to render each one. Outputs self-contained HTML + paired Markdown by default, with optional PDF, editable `.pptx`, and PlantUML `.puml`. Views: sequence (swimlane), topology (infrastructure + numbered segments), component (ports + protocol edges). Docs are in Korean.
...
| **검증** — 원본 대조 루프 (Chrome headless 캡처 + 원본 `qlmanage`/`pdftoppm`, Chrome 없으면 텍스트 대조 폴백) | ✅ |
...
/flowcast (오케스트레이터)
   ...
   └─ diagram-drawer 에이전트 × N  (run_in_background 병렬 팬아웃)
         ...
         → 해당 뷰 스킬 → 표준 JSON → HTML/MD → 선택적 PDF/PPT/PlantUML
```
...
drawer 반환은 단위 식별(`unit_id`·`system`·`source_ref`·`name`·`view`), 산출물 경로(`json`·`html`·`md`·`pdf`·`pptx`·`puml`), 페어 메타데이터(`pair_id`·`segment_numbers`), 실행 결과(`render_exit`·`warnings`·`error`·`questions`·`status`)로 구성된다. **필드의 단일 진실은 [`agents/diagram-drawer.md`](agents/diagram-drawer.md)의 출력 프로토콜**이다 — 여기에 JSON 예시를 복사해 두면 드리프트한다.

`status`는 `ok` / `partial` / `render_error` / `needs_input`이다. PDF를 요청했지만 Chrome이 없거나 PPT export를 요청했지만 python-pptx가 없으면 `partial`이며 HTML/MD는 유효하다.
