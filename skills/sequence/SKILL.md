...
- `bars`(최상위): `true` 면 액티베이션 바를 그린다. **기본은 그리지 않는다**
...
> **액티베이션 바를 기본으로 켜지 않는 이유** — 막대가 화살표의 시작·끝을 가리고 세로 시각
> 요소를 배로 늘려 흐름 자체가 덜 읽힌다. 2~3자 시퀀스에서는 대부분 전 구간이 활성이라
> 정보량도 거의 없다. 참여자가 많고 비동기 대기가 긴 흐름에서만 `"bars": true`로 켠다.
