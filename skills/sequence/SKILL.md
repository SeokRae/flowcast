...
- `scenarios[].steps[]`: `{ n?, from, to, label, kind, sub?, protocol?, tone? }`
- `kind`: `req`(실선) · `res`(점선 응답) · `relay`(중계, 이탤릭) · `self`(자기호출) · `note`(구간 구분선, label 필수)
- `tone`: 선의 **의미**. `danger`(취소·차단) · `warn`(환불·되돌림) · `info`(선택·조건부 단계)
- `actors[].tone`: 레인 색. `slate`(외부 사용자) · `violet`(자사 서비스) · `deep`(외부 기관)
- `bars`(최상위): `false` 면 액티베이션 바를 끈다. **기본은 그린다**(스텝 단위로 끊어서)
- `label` 개행(`\n`) = 다단 라벨. 빈 label의 `res` = 무라벨 응답 화살표. `n` 중복은 warning(원문 보존 허용).

> **`kind`와 `tone`은 직교한다.** 형태(실선/점선)와 의미(정상/되돌림)를 한 축에 섞으면
> "점선인 환불 응답" 같은 조합을 표현할 수 없고 조합마다 `kind`가 늘어난다.
> 되돌리는 동작(취소·환불·차단)이 정상 흐름과 섞여 있는 다이어그램에서만 `tone`을 쓴다 —
> 전부에 색을 칠하면 아무것도 강조되지 않는다.

> **액티베이션 바는 스텝 단위로 끊는다.** 받은 시점부터 자신이 다음으로 보내는 시점까지만
> 칠한다. 참여자별로 처음~마지막을 통으로 칠하면 2~3자 시퀀스에서 거의 항상 전 구간이 되어
> 굵은 라이프라인과 다를 바 없다. 막대가 방해되는 다이어그램에서는 `"bars": false`로 끈다.

> **`note`는 캔버스 전체 폭을 가로지르는 구분선이다.** 가운데에 라벨 칩이 얹혀 선을 끊는다.
> "이 지점을 경계로 아래는 조건부"를 뜻하므로 참여자 두 개 사이만 덮으면 그 두 레인에만
> 걸린 것처럼 읽힌다. 조건부 분기(`capture_method=manual` 같은)와 선택 처리 구간에 쓴다.

> **화살표는 막대 가장자리가 아니라 라이프라인까지 긋는다.** 막대에서 끊으면 선이 짧아 보이고
> 어디서 어디로 가는지가 덜 읽힌다. 화살표를 막대보다 나중에 그리므로 관통해도 위에 남는다.
...
6. 되돌리는 동작(취소·환불)이 정상 흐름과 섞여 있나? → 그 스텝에만 `tone`. 없으면 생략한다.
7. 스텝 **번호(n)** 매기나? 라벨 **다단(`\n`)**·**protocol**·**sub** 부가라벨 있나?
   - drawer가 입력 `segment_numbers`를 넘겼고 그게 비어 있지 않으면, **그 값을 순서대로 `n`에 그대로 쓰고 재부여하지 않는다**(topology 페어와 번호를 공유하기 위함 — 사후 훅이 대조한다). 원문상 구간을 더 쪼개야 하면 임의로 분할하지 말고 `needs_input`으로 확인을 넘긴다.
