...
### ① 데이터 수신 · 출력 경로 결정
사용자가 준 원문(텍스트/파일 경로)을 파악한다. `source`가 `.pptx`면 라우터가 `scripts/pptx_import.py`로 슬라이드별 draft(도형·라벨·좌표·커넥터)를 추출해 쪼갠다. **출력 디렉토리(`out_dir`)**를 정한다 — 사용자가 지정하지 않으면 현재 작업 디렉토리 하위 `flowcast-out/`를 기본으로 하고 한 줄로 알린다. Obsidian vault 등에서 iframe을 절대경로로 임베드하려면 `vault_iframe`(대상 절대경로 프리픽스)을 옵션으로 받는다.
