...
- `kind`: `srv`(기본) · `ext`(외부, 앰버) · `gear`(네트워크 장비, 점선) · `fw`(방화벽, 벽돌) · `l4`(L4/VIP 로드밸런서 — 좁은 점선 박스+fan-out, srv보다 경량) · `db`(데이터베이스 — 원통)
...
1. **노드 목록**과 각 노드 `kind`? (서버=srv / 외부 액터·시스템=ext / 라우터 등 장비=gear / **방화벽=fw** / **L4·VIP 로드밸런서=l4** / **DB·캐시·큐=db**)
