...
| 2026-08-08 | `flowcast:dataflow` 추가 — 데이터 파이프라인(ELT+Medallion) 도메인 프리셋. 질의 대본으로 흐름 문서를 먼저 만들어 소스 게이트를 충족시키고, 렌더는 `Skill` 도구로 `flowcast:component`에 위임(새 뷰 아님, 사용자 직접 호출 전용) | `skills/dataflow/`·`.claude-plugin/*`·`examples/analytics-elt-component.*`·`docs/*` | 파이프라인을 대화로만 아는 사용자가 component의 흐름 문서 요건에 매번 막히던 문제 해소 (#126) |
| 2026-08-08 | `flowcast:network-nat` 추가 — 네트워크/NAT 인프라 도메인 프리셋. 장비 IP·공인 IP·VIP·SNAT·DNAT·서버별 포트 방화벽 허용 표기 관례를 질의 대본으로 확정해 흐름 문서를 먼저 만들고, 렌더는 `Skill` 도구로 `flowcast:topology`에 위임(새 뷰 아님, 사용자 직접 호출 전용). topology 스키마 자체(`render.py`)는 변경하지 않음 — IP/NAT는 `name`/`meta` 자유텍스트 관례로만 표준화 | `skills/network-nat/`·`.claude-plugin/*`·`examples/gateway-nat-topology.*`·`docs/*` | 네트워크 구성을 대화로만 아는 사용자가 topology의 흐름 문서 요건에 매번 막히고, IP/VIP/NAT 표기가 매번 임의로 달라지던 문제 해소 |
| 2026-08-20 | `db` kind 추가 (component·topology) — DB·캐시·큐를 관례대로 원통(cylinder)으로 렌더. `_cyl_paths` 공용 헬퍼(몸통 path + 앞쪽 윗면 호), 라벨은 윗면 호를 피해 `_cyl_ry(h)/2` 만큼 하향. pptx는 `MSO_SHAPE.FLOWCHART_MAGNETIC_DISK`로 도형 일치, PlantUML은 기존 `<<kind>>` 스테레오타입으로 자동 전파 | `scripts/render.py`·`scripts/pptx_export.py`·`skills/{component,topology}/SKILL.md`·`examples/{microservice-component,three-tier-topology}.*` | 저장소를 사각형으로만 그릴 수 있어 인프라·시스템 구조도에서 DB가 서버와 구분되지 않았다. 실사용(payment-untact 시스템 구조도의 원장 DB)에서 확인 (#130) |
| 2026-08-20 | pptx export 다중 입력 — `pptx_export.py {a.json} {b.json} … -o deck.pptx` 로 입력 순서대로 슬라이드를 이어붙여 한 덱으로 병합(뷰 혼합 가능). exporter 3종에 `prs=None` 인자 추가(주입 시 append·저장 위임), 다중 입력은 한 장도 그리기 전에 전량 검증해 반쪽 덱을 남기지 않음. `--slide-size auto`는 덱마다 캔버스가 달라져 다중 입력에서 거부 | `scripts/pptx_export.py`·`CLAUDE.md`·`tests/test_pptx_cli.py` | 보고 자리에서 인프라 토폴로지와 서비스 아키텍처를 한 파일로 넘겨야 하는데 덱이 분리돼 파일 두 개를 열어야 했다. 실사용(payment-untact 보고)에서 확인 (#132) |
