...
def main():
    ap = argparse.ArgumentParser(description="flowcast component 뷰 → 편집가능 .pptx export")
    ...
    if data.get("view") != "component":
        print(f"이 export 는 component 뷰만 지원합니다 (view={data.get('view')!r}). "
              "topology·sequence 는 후속 예정.", file=sys.stderr)
        return 1
