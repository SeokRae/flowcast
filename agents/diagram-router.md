...
- **2축 페어링(흐름 데이터)**: 원문에 업무 서사(트리거→요청/응답→후속)가 있으면 한 뷰에 욱여넣지 않는다 — `sequence`(비즈니스 로직: 요청/응답 쌍·sync/async·시간 순서)와 `topology`(인프라 통신: 존·포트·경유 WEB/VIP) **두 단위 쌍**으로 제안한다. 두 단위에 같은 `pair_id`와 `segment_numbers`를 부여하고 페어링 근거를 `notes`에 남긴다. 업무 순서와 인프라 배치를 topology 한 장에 겹치면 둘 다 흐려진다. 사용자가 한 축만 원하면 그대로 따른다.
