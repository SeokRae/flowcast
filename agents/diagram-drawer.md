...
- 원본 파일이 있으면 뷰 스킬의 **원본 대조 검증** 루프를 수행한다. 브라우저 MCP가 아니라 **Bash(Chrome headless 캡처) + Read(이미지)** 로 한다 — 명령은 뷰 스킬에 있다. Chrome이 없으면 텍스트 1:1 대조로 폴백하고 `warnings`에 `visual-diff-skipped: {사유}`를 남기되 `status`는 `ok`를 유지한다(선택 출력 실패가 아니라 검증 수단의 폴백이다).
